  ...
  return (
    <div className="min-h-screen bg-background pb-24">
                      ...
                      {groupedEvents[date].map((event) => {
                        ...
                        return (
                          <div key={event.id} className="relative pl-6">
                            {/* Timeline dot */}
                            <div className={cn("absolute -left-[11px] top-4 w-5 h-5 rounded-full border-4 border-background", meta.colorClass.split(' ')[1], meta.colorClass.split(' ')[0].replace('text-', 'bg-'))} />
                            ...
                            <Card className={cn("overflow-hidden border-l-4", meta.colorClass.split(' ')[2])}>
                              <div className="flex justify-between items-start mb-2">
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <button 
                                    onClick={() => deleteEventMutation.mutate({ tripId, eventId: event.id })}
            ...
            <div className="space-y-6">
              ...
              <Card className="bg-gradient-to-br from-primary/5 to-primary/10 border-primary/20 p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                >
                  ...
                  {copied ? <CheckCircle2 className="w-5 h-5 text-emerald-500" /> : <Copy className="w-5 h-5 text-muted-foreground group-hover:text-primary transition-colors" />}
                </button>
              </Card>
      ...
      <AddEventModal 
        ...
        onAdd={(data) => createEventMutation.mutate({ tripId, data: { ...data, creatorId: userId! } })}
...
function AddEventModal({ isOpen, onClose, onAdd, isPending, tripStartDate, tripEndDate }: any) {
